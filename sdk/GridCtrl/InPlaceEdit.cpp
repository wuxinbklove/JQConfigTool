                           ...
                           UINT nFirstChar)
{
    ...
    m_nColumn = nColumn;
    m_nLastChar = 0;
    m_bExitOnArrows = (nFirstChar != VK_LBUTTON);    // If mouse click brought us here,then no exit on arrows
...
void CInPlaceEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (style & ES_MULTILINE == ES_MULTILINE)  //多行
    {
        int lineCount = str.Replace(_T("\n"), _T("\n")) + 1;
        int bottom = m_Rect.top + size.cy * lineCount + 8;  //新的高度
        if (nChar == VK_RETURN && m_Rect.bottom < bottom)  //输入回车，进行加高操作
        {
            m_Rect.bottom = bottom;
            MoveWindow(m_Rect);
            ((CGridCtrl*)GetParent())->SetRowHeight(m_nRow, m_Rect.Height());
        }
...
void CInPlaceEdit::EndEdit()
{
    ...
    bAlreadyEnding = TRUE;
    GetWindowText(str);
	if (this->GetStyle() & ES_MULTILINE == ES_MULTILINE)
	{
		str.Replace(_T("\r\n"), _T("\n"));
	}
