...
Flags(dwFlags)
{
    ...
    if (!sInitText.IsEmpty() && sInitText[0] == '.')
    {
        sInitText = _T("0") + sInitText;
    }
...
void CInPlaceNumEdit::OnUpdate()
{
    if (!Updating)
    {
        ...
        else
        {
            ...
            if (!LastValid.IsEmpty() && LastValid[0] == '.')
            {
                LastValid = _T("0") + LastValid;
            }
...
//    ASSERT (*EndPtr == NULL);
    ...
    if (*EndPtr == NULL)
    {
        ...
        if (count)
        {
            ...
            m_nNumber = Number;
            TheString = Formatted;
            if (!TheString.IsEmpty() && TheString[0] == '.')
            {
                TheString = _T("0") + TheString;
            }
            return TRUE;
        }
    }
