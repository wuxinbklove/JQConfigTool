...
bool _SplitString(CString& src, CString token, CStringArray& resultArray, bool ingoreEmptyString)
{
                     ...
                     COLORREF crBackClr /* CLR_DEFAULT */)
{
	...
	CClientDC dc(this);
	CString  lpszTitleText_t = CString(lpszTitleText);
	lpszTitleText_t.Replace(_T("\r\n"), _T("\n"));
	CStringArray showStrArr;
	_SplitString(lpszTitleText_t, _T("\n"), showStrArr, false);
	...
	for (size_t i = 0; i < showStrArr.GetCount(); i++)
	{
		CSize size = dc.GetTextExtent(CString(_T(" ")) + showStrArr[i] + CString(_T(" ")));
		if (maxTextWidth < size.cx)
			maxTextWidth = size.cx;
		maxTextHeight += size.cy + 2;
		singleLineHeight = size.cy;
	}
	maxTextHeight += 3;

    TEXTMETRIC tm;
    dc.GetTextMetrics(&tm);
	maxTextWidth += tm.tmOverhang;

	CRect rectDisplay = rectTitle;
	rectDisplay.left += xoffset;
	rectDisplay.right = rectDisplay.left + maxTextWidth + xoffset;
	rectDisplay.bottom = rectDisplay.top + maxTextHeight;
    
    // Do not display if the text fits within available space
    if ( rectDisplay.right > rectTitle.right-xoffset || rectDisplay.bottom > rectTitle.bottom + 3)
    {
		// Show the titletip
		...
		for (size_t i = 0; i < showStrArr.GetCount(); i++)
		{
			dc.TextOut(0, (i > 0 ? (singleLineHeight + 2) * i : 2), CString(_T(" ")) + showStrArr[i] + CString(_T(" ")));
		}
        SetCapture();
    }
