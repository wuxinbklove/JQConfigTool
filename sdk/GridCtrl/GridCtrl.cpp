...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	SetTitleTipBackClr(RGB(100, 100, 100));
	SetTitleTipTextClr(RGB(230, 230, 230));
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
		...
		//string text = GCharSet::MFC2STL(str);
		LPSTR text = T2A(str.GetBuffer());
        sf.Write(text, strlen(text));
        str.ReleaseBuffer();
    }
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	//LPTSTR buf = new TCHAR[::GlobalSize(hmem)];
    if (!szBuffer)
        return FALSE;

    sf.Read(szBuffer, ::GlobalSize(hmem));
	//sf.Read(buf, ::GlobalSize(hmem));
    ::GlobalUnlock(hmem);
...
BOOL CGridCtrl::SortTextItems(int nCol, BOOL bAscending, LPARAM data, int low, int hight)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
    ResetSelectedRange();
    SetFocusCell(-1, -1);
    return SortItems(pfnCellTextCompare, nCol, bAscending, data, low, hight);
}
...
BOOL CGridCtrl::SetColumnWidthPercent(int col, int percent)
{
    ...
    CRect rect;
    GetClientRect(rect);
    int width = (rect.Width() * percent) / 100;  //计算宽度值
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
    ...
    {
        CWaitCursor waiter;
        //modify by zhaosheng:把排序类型传给排序函数
        if (cell.col == GetSortColumn())
            SortItems(cell.col, !GetSortAscending(), !GetSortAscending());
        else
            SortItems(cell.col, TRUE, TRUE);
        Invalidate();
    }
